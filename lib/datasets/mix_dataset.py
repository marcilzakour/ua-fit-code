...
class MixWebDataset(wds.WebDataset):
    ...
    def __init__(self, cfg, dataset_list: list, max_len: int = None, data_preset: CN = None, is_train=True):
        ...
        curriculum_cfg = cfg.get("CURRICULUM_VIEWS", None)
        self.multiview_datasets = []
        self.datasets = []
        for sub_cfg in dataset_cfg_list:
            ds = MultiviewWebDataset(sub_cfg, data_preset, is_train, curriculum_cfg=curriculum_cfg)
            self.multiview_datasets.append(ds)
            self.datasets.append(ds.get_dataset())

        self.ratios = np.array([sub_cfg.MIX_RATIO for sub_cfg in dataset_cfg_list])
    ...
    def set_epoch(self, epoch):
        for ds in self.multiview_datasets:
            ds.set_epoch(epoch)
