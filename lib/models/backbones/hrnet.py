...
class HRNetw60(HighResolutionNet):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        super(HRNetw60, self).__init__(hrnet_config)
        self.init_weights(cfg.PRETRAINED)
        self.name = type(self).__name__
...
class HRNetw18(HighResolutionNet):
    """HRNetV2-W18 (channels 18/36/72/144) — efficiency-sweep backbone (~9.6M)."""

    def __init__(self, cfg, **kwargs):
        hrnet_yaml = "config/backbone/cls_hrnet_w18.yaml"
        hrnet_update_config(hrnet_config, hrnet_yaml)

        super(HRNetw18, self).__init__(hrnet_config)
        self.init_weights(cfg.PRETRAINED)
        self.name = type(self).__name__