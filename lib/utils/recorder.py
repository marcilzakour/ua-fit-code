...
class Recorder:
    ...
    def __init__(
        ...
        if not eval_only:
            assert (exp_id in ["default", "debug"]
                    or exp_id.startswith("debug_")
                    or exp_id.startswith("dev_")
                    or self.get_git_commit()), "MUST commit before the experiment!"
    ...
    @master_only
    def record_metric(self, metrics: List, epoch: int, comment="", section_label=""):
        """Write metrics to <comment>_Metric.txt.  section_label overrides the
        in-file header (useful to record train vs val in the same file)."""
        ...
        header = section_label if section_label else comment
        with open(metric_dump_path, "a") as f:
            f.write(f"Epoch {epoch} | {header} metric:\n")
            for M in metrics:
