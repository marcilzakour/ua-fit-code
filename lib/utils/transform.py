...
class RandomOcclusion:
    ...
    def __call__(self, results):
        ...
        img = results["image"]
        # decoded webdataset images are read-only; make writable before in-place fill
...
class SimpleTransform2D:
    ...
    def __init__(self, cfg: CN) -> None:
        ...
        # Constant crop-box zoom: multiply the stored bbox_scale so the crop covers more
        # (>1 = looser/more context, smaller hand) or less (<1 = tighter, bigger hand)
        # surrounding image, then resize to the SAME input res. Geometrically 3D-safe:
        # it folds into affine + target_cam_intr + target_joints_2d exactly (see below).
        # Applied in train AND eval; default 1.0 -> bit-for-bit reproduces existing runs.
        self._bbox_scale_mult = float(cfg.get("BBOX_SCALE_MULT", 1.0))
        ...
        # ── Extra PHOTOMETRIC augmentation (pixel-appearance only) ───────────────
        # IMPORTANT (3D safety): everything below perturbs pixel *values* and never
        # moves a pixel, so the 2D<->3D correspondence (target_joints_2d, target_cam_intr,
        # rot_mat3d, the later Theseus/DLT fitter) stays exactly valid. This is the ONLY
        # kind of augmentation safe to add here. Any *geometric* aug (extra crop/scale/
        # rotation/perspective/flip) MUST instead go through the existing rot/scale/center
        # path so it is folded into `affine`, `rot_mat3d` and `target_cam_intr`
        # (see SimpleTransform3DMultiView.__call__) — otherwise it silently corrupts 3D.
        # No HFlip: it turns a right hand into a left one, violating POEM's right-hand
        # only protocol (train + eval). All knobs default to 0/off → existing configs
        # reproduce bit-for-bit.
        self._contrast_jit = cfg.get("CONTRAST_JIT", 0.0) if self._aug else 0.0
        self._saturation_jit = cfg.get("SATURATION_JIT", 0.0) if self._aug else 0.0
        self._hue_jit = cfg.get("HUE_JIT", 0.0) if self._aug else 0.0            # frac of 180deg, <=0.5
        self._gamma_jit = cfg.get("GAMMA_JIT", 0.0) if self._aug else 0.0
        self._blur_prob = cfg.get("BLUR_PROB", 0.0) if self._aug else 0.0
        self._blur_sigma = cfg.get("BLUR_SIGMA", [0.1, 1.5])
        self._noise_prob = cfg.get("NOISE_PROB", 0.0) if self._aug else 0.0
        self._noise_std = cfg.get("NOISE_STD", 8.0)                              # in 0-255 units
        self._gray_prob = cfg.get("GRAY_PROB", 0.0) if self._aug else 0.0
        self._jpeg_prob = cfg.get("JPEG_PROB", 0.0) if self._aug else 0.0
        self._jpeg_quality = cfg.get("JPEG_QUALITY", [40, 95])
    ...
    def _photometric(self, image):
        """Pixel-appearance-only augmentation. 3D-safe: never moves a pixel, so all
        geometry (2D joints, target_cam_intr, rot_mat3d, downstream fitter) is untouched.
        Args/returns: HxWx3 uint8 RGB image (decoded as 'rgb8')."""
        img = image
        if self._contrast_jit > 0:                                   # contrast about per-image mean
            f = 1.0 + random.uniform(-self._contrast_jit, self._contrast_jit)
            mean = img.reshape(-1, 3).mean(axis=0)
            img = np.clip((img.astype(np.float32) - mean) * f + mean, 0, 255).astype(np.uint8)
        if self._saturation_jit > 0 or self._hue_jit > 0:            # saturation / hue in HSV
            hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV).astype(np.float32)
            if self._saturation_jit > 0:
                hsv[:, :, 1] *= (1.0 + random.uniform(-self._saturation_jit, self._saturation_jit))
            if self._hue_jit > 0:
                hsv[:, :, 0] = np.mod(hsv[:, :, 0] + random.uniform(-self._hue_jit, self._hue_jit) * 180.0, 180.0)
            hsv[:, :, 1] = np.clip(hsv[:, :, 1], 0, 255)
            img = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2RGB)
        if self._gamma_jit > 0:                                      # gamma / illumination
            g = 1.0 + random.uniform(-self._gamma_jit, self._gamma_jit)
            img = np.clip(((img.astype(np.float32) / 255.0) ** g) * 255.0, 0, 255).astype(np.uint8)
        if self._gray_prob > 0 and random.random() < self._gray_prob:  # drop colour cue
            img = np.repeat(cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)[:, :, None], 3, axis=2)
        if self._blur_prob > 0 and random.random() < self._blur_prob:  # defocus
            sigma = random.uniform(self._blur_sigma[0], self._blur_sigma[1])
            img = cv2.GaussianBlur(img, (0, 0), sigmaX=sigma)
        if self._noise_prob > 0 and random.random() < self._noise_prob:  # sensor noise
            img = np.clip(img.astype(np.float32) + np.random.randn(*img.shape) * self._noise_std,
                          0, 255).astype(np.uint8)
        if self._jpeg_prob > 0 and random.random() < self._jpeg_prob:  # codec artefacts
            q = int(random.uniform(self._jpeg_quality[0], self._jpeg_quality[1]))
    ...
    def __call__(self, image, label, **kwargs):
        if self._aug:
            ...
            c_factor = np.random.normal(loc=0, scale=cf, size=2)
            bbox_center = label["bbox_center"] + c_factor * label["bbox_scale"]
            s_factor = np.random.normal(loc=1, scale=sf)
            bbox_scale = label["bbox_scale"] * s_factor * self._bbox_scale_mult

            r_factor = np.random.normal(loc=0, scale=rf)
            no_rot = kwargs.get("no_rot", False)
            if not no_rot and np.random.rand() <= self._rot_prob:
                rot = np.deg2rad(r_factor)
            else:
                rot = 0.0
            ...
            bbox_scale = label["bbox_scale"] * self._bbox_scale_mult
...
class SimpleTransformUVD(SimpleTransform2D):
    ...
    def __call__(self, image, label, **kwargs):
        results = super().__call__(image, label, **kwargs)
...
@TRANSFORM.register_module()
class SimpleTransform3DMultiView(SimpleTransform2D):
    ...
    def __call__(self, image, label, **kwargs):
        ...
        # MANO shape GT (rotation-invariant, passthrough) — beta-head supervision.
        if "mano_shape" in label:
            results["target_mano_shape"] = np.asarray(label["mano_shape"], dtype=np.float32)

        return results


@TRANSFORM.register_module()
class SimpleTransform3D(SimpleTransform2D):
