...
from .utils import ColorsHandJoints
...
def draw_batch_mesh_images(verts3d, gt_verts3d, face, intr, tensor_image, step_idx, n_sample=16):
    ...
    image = image.mul_(255.0).float().numpy().astype(np.uint8)  # (B, H, W, 3)

    verts3d = verts3d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
    gt_verts3d = gt_verts3d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
    intr = intr[:n_sample, ...].detach().cpu().float().numpy()  # (B, 3, 3)
...
def draw_batch_verts_images(verts2d, gt_verts2d, tensor_image, step_idx, n_sample=16):
    ...
    verts2d = verts2d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
    gt_verts2d = gt_verts2d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
...
def draw_batch_joint_images(joints2d, gt_jointd2d, tensor_image, step_idx, n_sample=16):
    ...
    joints2d = joints2d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
    gt_jointd2d = gt_jointd2d[:n_sample, ...].detach().cpu().float().numpy()  # (B, NJ, 2)
...
def plot_hand(image, coords_hw, vis=None, linewidth=3):
    ...
    colors = np.array(ColorsHandJoints)
    colors = colors[:, ::-1]
    ...
    if joints_uv is not None:
        ...
        for joint_ind in range(joints_uv.shape[0]):
            ...
            cv2.circle(
                ...
                joint,
                radius=marker_sz,
                color=ColorsHandJoints[joint_ind] * np.array(255),
                thickness=-1,
        ...
        for joint_ind in range(joints_xyz.shape[0]):
            ...
            if joint_ind == 0:
                ...
                ax.plot(
                    ...
                    color=ColorsHandJoints[joint_ind],
                    linewidth=line_wd,
                )
            else:
                ax.plot(
...
def save_a_image_with_mesh_joints(
    ...
    img_list = [image, skeleton_overlay, rend_img_overlay]
    if with_mayavi_mesh:
