...
class _PCKMetric(Metric, ABC):
    ...
    def feed(self, preds: Dict, targs: Dict, **kwargs):
        ...
        assert len(kp_preds.shape) == 3, logger.error("X kp_preds shape, got {}".format(kp_preds.shape))  # ARRAY (BATCH, NPOINTS, 3)
        assert len(kp_targs.shape) == 3, logger.error("X kp_targs shape, got {}".format(kp_targs.shape))  # ARRAY (BATCH, NPOINTS, 3)
        assert len(kp_vis.shape) == 2, logger.error("X kp_vis shape, got {}".format(kp_vis.shape))  # ARRAY (BATCH, NPOINTS)
    ...
    def get_measures(self) -> Dict:
        ...
        area_under_one = np.trapezoid(np.ones_like(thresholds), thresholds)
        ...
        for kp_id in range(self.num_kp):
            ...
            auc = np.trapezoid(pck_curve, thresholds)
