...
class PAEval(Metric):
    ...
    def feed(self, pred_joints_3d_abs, joints_3d_abs, pred_verts_3d_abs=None, verts_3d_abs=None, **kwargs):
        ...
        # Robustness: a non-finite prediction (early-training transient) must not kill the
        # run — scipy's procrustes raises on inf/NaN. Replace with a large finite value so
        # the sample simply scores badly (correct semantics) instead of crashing a 4h job.
        if not np.isfinite(pred_joints_3d_abs).all():
            pred_joints_3d_abs = np.nan_to_num(pred_joints_3d_abs, nan=1.0, posinf=1.0, neginf=-1.0)
